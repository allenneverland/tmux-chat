...
const HOOK_NOTIFY_COMMAND: &str = "tmux-chatd notify";
const CODEX_NOTIFY_LINE: &str = "notify = [\"tmux-chatd\", \"notify\"]";

#[derive(Parser)]
#[command(name = "tmux-chatd")]
#[command(version)]
...
fn get_data_dir() -> std::path::PathBuf {
    std::env::var("TMUX_CHATD_DATA_DIR")
    ...
    tracing_subscriber::registry()
        .with(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "tmux-chatd=debug".into()),
        )
    ...
    match action {
        ...
        Some(DeviceAction::List) | None => {
            ...
            if devices.is_empty() {
                ...
                println!("\nRun 'tmux-chatd devices issue --name <DEVICE_NAME> --json' to issue a device token");
            } else {
...
fn install_codex_hooks() {
    ...
    if has_other_notify {
        ...
        println!("Add TmuxChat manually if needed: {}", CODEX_NOTIFY_LINE);
        return;
    }
    ...
    let filtered: Vec<&str> = existing
        ...
        .filter(|line| {
            let t = line.trim();
            t != "# TmuxChat push notification hook" && t != CODEX_NOTIFY_LINE
        })
        .collect();
    let mut out = format!("# TmuxChat push notification hook\n{}\n", CODEX_NOTIFY_LINE);
...
async fn run_notify_command(
        ...
        .await
    {
        ...
        Err(e) => {
            eprintln!("Failed to connect to tmux-chatd: {}", e);
            eprintln!("Make sure tmux-chatd daemon is running on port {}", port);
            std::process::exit(1);
        }
    }
}
...
async fn run_daemon(data_dir: std::path::PathBuf) {
    ...
    if !auth_service.has_devices().await {
        tracing::warn!(
            "No devices registered. Run 'tmux-chatd devices issue --name <DEVICE_NAME> --json' to issue a device token."
        );
    }
    ...
    let base_routes = Router::new()
        ...
        .route("/panes/{target}/input-events", post(api::send_input_events))
        .route("/panes/{target}/key", post(api::send_key_legacy))
        .route("/panes/{target}/keys", post(api::send_keys_legacy))
