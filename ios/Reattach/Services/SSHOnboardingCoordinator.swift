...
final class SSHOnboardingCoordinator {
    ...
    var stepLabel: String {
        ...
        case .verifyingReattachd:
            return "Checking reattachd on host"
        case .detectingPlatform:
    ...
    func start(input: SSHOnboardingInput, apnsToken: String?) async -> Bool {
        ...
        do {
            ...
            step = .verifyingReattachd
            do {
                try await installer.ensureReattachdInstalled(on: connectionSpec)
            } catch {
                throw HostAgentInstallerError.missingReattachd
            }
    ...
    private func issueDeviceCredentials(on connection: SSHConnectionSpec) async throws -> IssuedDeviceCredentials {
        ...
        let command = "reattachd devices issue --name \(shellQuote(rawDeviceName)) --json"
